            ...
            switch (e.type) {
                ...
                case ButtonPress:
                    if (e.xbutton.button == 1) { //Left mouse Button
                        if (win == g->mainWindow)
                            clicked = get_at_coords(g->widgets, e.xbutton.x,
                                    e.xbutton.y);
                        else {
                            win_data = list_get(g->windows, window_comp, &win);
                            if (win_data != NULL)
                                clicked = get_at_coords(win_data->widgets,
                        ...
                        if (clicked != NULL) {
                            ...
                            if ((clicked->flags & (CLICKABLE | SELECTABLE))
                                    > 0) {
                                active = clicked;
                ...
                case MapNotify:
                    if (win == g->mainWindow) {
                        ...
                        while ((temp = (WIDGET*) sorted_list_get_next(
                                g->widgets)) != NULL) {
...
void create_main_window(GUI* g, char* title)
{
    ...
    w = XCreateSimpleWindow(g->dsp, DefaultRootWindow(g->dsp), 0, 0, 100, 100,
            0, 0, g->bgColor);
    XSelectInput(g->dsp, w,
            StructureNotifyMask | KeyReleaseMask | ButtonPressMask
...
void set_main_icon(GUI* g, char* filename)
{
    ...
    if (strstr(filename, ".xpm") != NULL) {
        if (access(filename, R_OK) != -1) {
            XpmReadFileToPixmap(g->dsp, g->mainWindow, filename, &p, NULL,
                    NULL);
            hint = XAllocWMHints();
...
WIDGET* get_at_coords(SORTED_LIST* widgets, int x, int y)
{
    ...
    while ((temp = sorted_list_get_next(widgets)) != NULL) {
        if ((temp->flags != NONE) && ((temp->status & STATUS_VISIBLE) > 0)
                && ((temp->status & STATUS_ENABLE) > 0)) {
...
{
    char re = 0;
    KeySym key_symbol = XkbKeycodeToKeysym(g->dsp, e->keycode, 0,
