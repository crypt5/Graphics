...
WINDOW* create_window(GUI* g, char* title, int bgColor)
{
    ...
    w->w = XCreateSimpleWindow(g->dsp, DefaultRootWindow(g->dsp), 0, 0, 100,
            100, 0, 0, bg);
    XSelectInput(g->dsp, w->w,
...
void ok_popup_callback(GUI* g, WIDGET* w, void* data)
{
    *(int*) data = 1;
}
int ok_popup(GUI* g, char* message, char* title, int popup_type)
{
    ...
    XChangeProperty(g->dsp, win->w, desc, XA_ATOM, 32, PropModeReplace,
